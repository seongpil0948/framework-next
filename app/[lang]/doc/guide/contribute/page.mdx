...
3. 이슈에 대한 브랜치 생성 (create branch)
   1. 브랜치 이름은 kebab case 를 준수한다.
   2. `[{issue number}]-[{issue type}]-[{issue name}]`
4. Remote 저장소에 자동 생성된 브랜치의 fetch or pull(origin) 진행한다.
5. 로컬 저장소에서 코드 수정 및 커밋을 진행한다.
6. Git squash 로 커밋 메시지 정리한다. [rebase 를 이용한 squash](https://meetup.nhncloud.com/posts/39)
7. Git rebase 로 로컬 작업 결과를 최신화 한다.
8. 병합 대상 브랜치로 Merge Request를 생성한다.
9. maintainer 권한을 가진 인원( 최소 2명 이상)의  리뷰 이후  approve를 진행한다.
